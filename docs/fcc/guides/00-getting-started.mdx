...
How those signatures work is split in two:
...
This guide uses the Go default (`LANGUAGE=go` in `.env`).
Set `LANGUAGE=python` or `LANGUAGE=typescript` to run the other implementations; the contract, scripts, and Coston2 flow stay the same.
...
Customize the Go files under `go/` so OPType and OPCommand strings stay aligned with the Solidity contract:

| #   | File                                 | What you change                                            |
| --- | ------------------------------------ | ---------------------------------------------------------- |
| 1   | `go/internal/config/config.go`       | OPType and OPCommand string constants and SemVer `Version` |
| 2   | `go/pkg/types/types.go`              | Request, response, and state structs                       |
| 3   | `go/internal/extension/extension.go` | Routing and handlers                                       |
| 4   | `contracts/InstructionSender.sol`    | Matching `bytes32` constants and send functions            |
| 5   | `tools/cmd/run-test/main.go`         | E2E payloads and assertions                                |
...
Edit the `[db]` block in `config/proxy/extension_proxy.coston2.docker.toml` with the Coston2 indexer host and the read-only credentials provided to you:
...
3. Implement handlers and types.
