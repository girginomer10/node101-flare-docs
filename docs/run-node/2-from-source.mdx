...
Read more about them in the [AvalancheGo configs and flags](https://github.com/ava-labs/avalanchego/blob/master/config/config.md) documentation.

{/* prettier-ignore */}
- [`--bootstrap-ips`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#bootstrapping),
  [`--bootstrap-ids`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#bootstrapping):
...
- [`--http-host`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#http-server): Use `--http-host=` (empty) to allow connections from other machines.
Otherwise, only connections from `localhost` are accepted.

- [`--http-port`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#http-server): The port through which the node will listen to API requests.
The default value is `9650`.

- [`--http-allowed-hosts`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#http-server): Use `--http-allowed-hosts="*"` to allow connections from any machine.
Otherwise, only connections from `localhost` are accepted.

- [`--staking-port`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#staking): The port through which the network peers will connect to this node externally.
Having this port accessible from the internet is required for correct node operation.
The default value is `9651`.

- [`--db-dir`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#database): Directory where the database is stored, defaults to `~/.avalanchego/db`.
...
- [`--chain-config-dir`](https://github.com/ava-labs/avalanchego/blob/master/config/config.md#chain-configuration): Optional JSON configuration file for using non-default values.
